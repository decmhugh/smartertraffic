    ...
    if len(record) > 0:
        ...
        for newitem in data[d]["item"]:
            ...
            for i,res in enumerate(r["item"]):
                if newitem["item_id"].startswith(r["item"][i]["item_id"]):
                    remove_old.append(newitem)
                    r["item"][i]=newitem
                    exist = True
            if not exist:
                remove_old.append(newitem)
        ...
        for item in data[d]["item"]:
            remove_old.append(item)
        cnn_local.save(data[d])
for old_item in remove_old:
    print("removing ",old_item["item_id"])
    db_remote.twitter_streaming.remove({"item_id":old_item["item_id"]})
